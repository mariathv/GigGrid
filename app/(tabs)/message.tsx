...
export default function MessagesScreen() {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                const mappedConversations = data.data.chats.map((chat: Chat) => {
                    // For freelancer view, the other person is the client
                    const unreadCount = chat.messages.filter(
                        (msg: Message) => !msg.read && msg.sender === chat.client._id
                    ).length
    ...
    useEffect(() => {
        ...
        // If there's a clientId in params, navigate to that conversation
        if (params.clientId) {
            const conversation = conversations.find(c => c.clientId === params.clientId)
            if (conversation) {
                router.push({
                    pathname: "/chat",
                    params: {
                        ...
                        >
                            ...
                            <View style={styles.conversationContent}>
                                <View style={styles.conversationHeader}>
                                    <ThemedText style={styles.freelancerName}>{conversation.clientName}</ThemedText>
                    ...
                    <View style={styles.emptyStateContainer}>
                        ...
                        <ThemedText style={styles.emptyStateText}>
                            {searchQuery.trim() !== ""
                                ...
                                : "You don't have any active conversations with clients."}
                        </ThemedText>
                        {searchQuery.trim() !== "" && (
                            <TouchableOpacity
